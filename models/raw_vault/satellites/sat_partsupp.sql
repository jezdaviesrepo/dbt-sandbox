{{ config(
    ...
    post_hook="{% if not is_incremental() %}ALTER TABLE {{ this }} ADD CONSTRAINT pk_{{ this.name }} PRIMARY KEY (PART_SUPPLIER_HK, LOAD_DATE){% endif %}"
) }}

{%- set source_model = "stg_partsupp"           -%}
{%- set src_pk       = "PART_SUPPLIER_HK"       -%}
{%- set src_hashdiff = "PART_SUPPLIER_HASHDIFF" -%}
